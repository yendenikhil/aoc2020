...
const rule1 = (graph: string[][], row: number, column: number) => {
  return isEmptySeat(graph, row, column) &&
    adjucentSeats(graph, row, column).filter((s) => s === "#").length === 0;
};
const rule1V2 = (graph: string[][], row: number, column: number) => {
  return isEmptySeat(graph, row, column) &&
    adjucentSeatsV2(graph, row, column).filter((s) => s === "#").length === 0;
};
const rule2 = (graph: string[][], row: number, column: number) => {
  return isOccSeat(graph, row, column) &&
    adjucentSeats(graph, row, column).filter((s) => s === "#").length > 3;
};
const rule2V2 = (graph: string[][], row: number, column: number) => {
  return isOccSeat(graph, row, column) &&
    adjucentSeatsV2(graph, row, column).filter((s) => s === "#").length > 4;
};
...
const part1 = (graph: string[][]) => {
  ...
  while (!match) {
    ...
    for (let i = 0; i < newGraph.length; i++) {
      for (let j = 0; j < newGraph[0].length; j++) {
        if (rule1(startGraph, i, j) || rule2(startGraph, i, j)) {
          changeSeatState(newGraph, i, j);
        }
      }
    }
  ...
  p(
    startGraph.map((r) => r.filter((s) => s === "#").length).reduce((a, b) =>
      a + b
    ),
  );
};
...
const part2 = (graph: string[][]) => {
  ...
  while (!match) {
    ...
    for (let i = 0; i < newGraph.length; i++) {
      for (let j = 0; j < newGraph[0].length; j++) {
        if (
          rule1V2(startGraph, i, j) || rule2V2(startGraph, i, j)
        ) {
          changeSeatState(newGraph, i, j);
        }
      }
